...
def initialize_networks(input_dim, num_actions, num_agents):
    ...
    """
    q_networks = {f"agent_{i}": QNetwork(input_dim, num_actions) for i in range(num_agents)}
    target_networks = {f"agent_{i}": QNetwork(input_dim, num_actions) for i in range(num_agents)}
    for i in range(num_agents):
        target_networks[f"agent_{i}"].load_state_dict(q_networks[f"agent_{i}"].state_dict())
...
q_networks = {
    f"agent{i}": QNetwork(int(input_dim), num_actions) for i in range(env._num_agents)
}
...
replay_buffer = ReplayBuffer(1000000)  # Adjust capacity as needed
    ...
    for episode in range(num_episodes):
        ...
        for i in range(env._num_agents):
            for target_param, local_param in zip(
                target_networks[f"agent{i}"].parameters(),
                q_networks[f"agent{i}"].parameters(),
            ):
        ...
        if stop_training:
            print(
                f"Stopping training, average reward {average_reward} reached at episode {episode}"
            )
            break
